...
export const BetCalculator = () => {
  ...
  const saveResult = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (coef > 0 && value > 0 && betType !== null) {
      ...
      setErrorMsg('Please enter all fields: bet amount, coefficient and bet type');
    }
  };
    ...
    <main className="bet-calculator-bg">
      <div className="bet-calculator">
        <form className="bet-form" onSubmit={(e) => e.preventDefault()}>
          ...
          {errorMsg && <p className="bet-calc-error">{errorMsg}</p>}
                  ...
                  {history.map((item) => (
                    <tr key={item.date}>
                      ...
                      <td className="type_cell">
                        {item.betType === 0 ? 'Sport' : item.betType === 1 ? 'Slot' : 'Card game'}
                      </td>
